...
export default function Nav() {
  ...
  const messageNotificationTypes = ['chat_message', 'teacher_message', 'student_message', 'group_message'];
  const messageNotifications = notifications?.notifications?.filter(notif => 
    messageNotificationTypes.includes(notif.type)
  ) || [];
  const otherNotifications = notifications?.notifications?.filter(notif => 
    !messageNotificationTypes.includes(notif.type)
  ) || [];
  
  const messageUnreadCount = messageNotifications.filter(notif => !notif.is_read).length;

  // Function to render a single notification
  const renderNotification = (notification, index, total) => (
    <Box
      key={notification.id}
      p={4}
      ...
      <HStack spacing={3} align="flex-start">
        ...
        <VStack align="flex-start" spacing={3} flex={1}>
          <VStack align="flex-start" spacing={2} w="full">
            ...
            {/* عرض معلومات إضافية لإشعارات الرسائل */}
            {messageNotificationTypes.includes(notification.type) && (
              <VStack align="flex-start" spacing={1} w="full" mt={1}>
                {notification.group_name && (
                  <Badge colorScheme="green" fontSize="xs" px={2} py={1} borderRadius="md">
                    المجموعة: {notification.group_name}
                  </Badge>
                )}
                ...
                {notification.metadata?.last_message && (
                  <Text fontSize="xs" color={useColorModeValue("gray.600", "gray.300")} fontStyle="italic">
                    "{notification.metadata.last_message}"
                  </Text>
                )}
              </VStack>
            ...
            <HStack spacing={2} w="full" flexWrap="wrap">
              ...
              >
                {notification.type === 'lecture_added' ? 'محاضرة' :
                 ...
                 notification.type === 'teacher_message' ? 'رسالة مدرس' :
                 notification.type === 'student_message' ? 'رسالة طالب' :
                 notification.type === 'group_message' ? 'رسالة مجموعة' :
                 notification.type === 'social_comment' ? 'تعليق' :
                 notification.type === 'social_reply' ? 'رد' :
                 notification.type === 'social_reaction' ? 'تفاعل' :
                 notification.type === 'social_like' ? 'إعجاب' :
                          ...
                          <HStack spacing={3}>
                            ...
                            <VStack align="flex-start" spacing={0}>
                              ...
                              <Text fontSize="xs" color="gray.500">
                                {messageUnreadCount > 0 ? `${messageUnreadCount} رسالة جديدة` : 'لا توجد رسائل جديدة'}
                              </Text>
                            </VStack>
                          </HStack>
                        </HStack>
                      ...
                      <PopoverBody maxH='400px' overflowY='auto' p={0}>
                        {notificationsLoading ? (
                          <VStack spacing={4} py={6}>
                            <Icon as={FaComments} color="gray.400" boxSize={8} />
                            <Text color="gray.500" fontSize="sm">جاري تحميل الرسائل...</Text>
                          </VStack>
                        ) : messageNotifications.length > 0 ? (
                          <VStack spacing={0} align="stretch">
                            {messageNotifications.map((notification, index) => renderNotification(notification, index, messageNotifications.length))}
                          </VStack>
                        ) : (
                          <VStack spacing={6} py={12}>
                            ...
                            <VStack spacing={2}>
                              ...
                              <Text 
                                ...
                                maxW="200px"
                              >
                                ستظهر رسائلك الجديدة هنا عند وصولها
                              </Text>
                            </VStack>
                          </VStack>
                        )}
                    ...
                    >
                        ...
                        <HStack spacing={3} justify="space-between">
                          <HStack spacing={3}>
                            ...
                            <VStack align="flex-start" spacing={0}>
                              ...
                              <Text fontSize="xs" color="gray.500">
                                {(unreadCount - messageUnreadCount) > 0 ? `${unreadCount - messageUnreadCount} إشعار جديد` : 'لا توجد إشعارات جديدة'}
                              </Text>
                            </VStack>
                          </HStack>
                      ...
                      <PopoverBody maxH='400px' overflowY='auto' p={0}>
                        {notificationsLoading ? (
                          ...
                          <VStack spacing={0} align="stretch">
                            {otherNotifications.map((notification, index) => renderNotification(notification, index, otherNotifications.length))}
                          </VStack>
                        ) : (
                          <VStack spacing={6} py={12}>
                            <Box
