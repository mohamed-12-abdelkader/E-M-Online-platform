...
const PlatfourmLeagues = () => {
  ...
  const headerBgClass = useColorModeValue('bg-white/80 border-slate-200/60', 'bg-slate-900/70 border-slate-700/60')
  const cardBgClass = useColorModeValue('bg-white/70 border-slate-200/60', 'bg-slate-800/60 border-slate-700/60')
  ...
  const modalBorderClass = useColorModeValue('border-white/20', 'border-white/10')
  const sectionBgClass = useColorModeValue('bg-slate-50/50', 'bg-slate-800/50')
  const inputBorderClass = useColorModeValue('border-slate-300', 'border-slate-600')
  ...
  const fetchTournaments = useCallback(async () => {
    try {
      setLoading(true)
      setError("")
      const endpoint = userData?.role === 'student' ? '/api/leagues/student' : '/api/leagues'
  ...
  const handleDelete = async (tournamentId) => {
    try {
      setDeleting(true)
      await baseUrl.delete(`/api/leagues/${tournamentId}`, { headers: authHeader })
      await fetchTournaments()
  ...
  const handleFreeJoin = async () => {
    ...
    try {
      ...
      setJoinError("")
      setJoinMsg("")
      await baseUrl.post(`/api/leagues/${joinLeague.id}/join`, {}, { headers: authHeader })
  ...
  return (
    <div className="min-h-screen" style={{ background: pageBg }}>
      {/* Header Section */}
      <div className={`backdrop-blur-sm border-b sticky top-0 z-40 ${headerBgClass}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap">
          {tournaments.map((t, index) => (
            <div
              key={t.id}
              className={`group m-3 ${cardBgClass} backdrop-blur-sm border rounded-2xl shadow-sm hover:shadow-xl hover:shadow-indigo-500/10 transition-all duration-500 transform hover:-translate-y-1 overflow-hidden w-full`}
              style={{ width: "370px" }}
            >
              {/* Image Section */}
              ...
              <div className="relative overflow-hidden">
                {t.image_url ? (
                  <img src={t.image_url} alt={t.name} className="w-full h-[200px] object-cover group-hover:scale-110 transition-transform duration-500" />
                ) : (
                ...
                <div className="absolute top-3 left-3 space-y-1">
                  {typeof t.is_visible !== 'undefined' && (
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium backdrop-blur-sm ${t.is_visible ? 'bg-blue-500/90 text-white' : 'bg-gray-500/90 text-white'
                      }`}>
                      {t.is_visible ? 'ظاهر' : 'مخفي'}
                    </span>
                  )}
                  {typeof t.is_subscribed !== 'undefined' && (
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium backdrop-blur-sm ${t.is_subscribed ? 'bg-emerald-500/90 text-white' : 'bg-slate-500/90 text-white'
                      }`}>
                  ...
                  {isAdmin ? (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-2.5 px-4 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-lg font-medium transition-all duration-300 hover:shadow-lg flex items-center justify-center gap-2"
                      >
                    ...
                    userData?.role === 'student' ? (
                      t.is_enrolled ? (
                        <Link to={`/league/${t.id}`}>
                          ...
                          <button className="w-full py-2.5 px-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg font-medium transition-all duration-300 hover:shadow-lg">
                            عرض التفاصيل
                          </button>
                        </Link>
                      ) : (
                        <button
                          ...
                          className="w-full py-2.5 px-4 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-lg font-medium transition-all duration-300 hover:shadow-lg"
                        >
                          اشترك
                        </button>
                      )
        ...
        {isOpen && (
          <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[100px]">
            ...
            />
            <div className={`relative ${modalBgClass} backdrop-blur-sm w-full max-w-3xl rounded-3xl shadow-2xl border ${modalBorderClass} animate-scale-in overflow-hidden max-h-[90vh] overflow-y-auto`}>
              {/* Modal Header */}
              <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-4 sm:px-6 md:px-8 py-4 sm:py-5 md:py-6 text-white">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3 space-x-reverse">
              ...
              <div className="px-4 sm:px-6 md:px-8 py-4 sm:py-5 md:py-6">
                <form id="create-league-form" onSubmit={editMode ? handleUpdate : handleCreate} className="space-y-6">
                  {/* Tournament Name */}
                  ...
                  {/* Pricing Section */}
                  <div className={`${sectionBgClass} rounded-2xl p-6 border ${useColorModeValue('border-slate-200/50', 'border-slate-700/50')}`}>
                  ...
                  <div className="space-y-2">
                    ...
                    {(imagePreview || (editMode && editingTournament?.image_url)) && (
                      <div className="mt-3 relative">
                        ...
                        {imagePreview && (
                          <div className="mt-3 flex items-center justify-end">
                            <button
                              type="button"
                              onClick={() => { setImageFile(null); setImagePreview(""); try { if (fileInputRef?.current) fileInputRef.current.value = "" } catch { } }}
              ...
              {/* Modal Footer */}
              <div className={`${sectionBgClass} px-4 sm:px-6 md:px-8 py-4 sm:py-5 md:py-6 border-t ${useColorModeValue('border-slate-200/50', 'border-slate-700/50')}`}>
        ...
        {isJoinOpen && joinLeague && (
          <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[100px]">
            ...
            />
            <div className={`relative ${modalBgClass} backdrop-blur-sm w-full max-w-lg rounded-3xl shadow-2xl border ${modalBorderClass} animate-scale-in overflow-hidden`}>
              {/* Header */}
              <div className="bg-gradient-to-r from-emerald-600 to-green-600 px-4 sm:px-6 py-4 text-white">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3 space-x-reverse">
                    <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 01-8 0M3 11h18M5 11v8a2 2 0 002 2h10a2 2 0 002-2v-8" />
                      </svg>
                    </div>
                    <div>
                      <h3 className="text-xl font-bold">الاشتراك في الدوري</h3>
                      <p className="text-white/80 text-sm mt-1">{joinLeague?.name}</p>
                    </div>
                  </div>
                  <button
                    onClick={closeJoinModal}
              ...
              <div className="px-4 sm:px-6 py-5">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-slate-800 font-semibold">{formatPrice(joinLeague?.price)}</span>
                  </div>

                  {joinLeague?.price == null ? (
                    <div className={`mt-2 ${sectionBgClass} rounded-2xl p-4 border ${useColorModeValue('border-slate-200/50', 'border-slate-700/50')}`}>
                      <div className="flex items-center gap-2 text-slate-700">
                        <svg className="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                        </svg>
                        <span className="text-sm">هذا الدوري مجاني. هل ترغب في تأكيد الاشتراك؟</span>
                      </div>
                    </div>
                  ) : (
                    <div className={`grid grid-cols-1 sm:grid-cols-2 gap-3 ${sectionBgClass} rounded-2xl p-4 border ${useColorModeValue('border-slate-200/50', 'border-slate-700/50')}`}>
                      <button
                        type="button"
              ...
              {/* Footer */}
              <div className={`${sectionBgClass} px-4 sm:px-6 py-4 border-t ${useColorModeValue('border-slate-200/50', 'border-slate-700/50')}`}>
        ...
        {deleteConfirm && (
          <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[100px]">
            ...
            />
            <div className={`relative ${modalBgClass} backdrop-blur-sm w-full max-w-md rounded-3xl shadow-2xl border ${modalBorderClass} animate-scale-in overflow-hidden max-h-[90vh] overflow-y-auto`}>
              {/* Delete Modal Header */}
