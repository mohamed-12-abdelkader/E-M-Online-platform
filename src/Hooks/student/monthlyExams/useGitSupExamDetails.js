...
const fetchExamsData = async (token, id) => {
  const response = await baseUrl.get(`api/exams//${id}/`, {
    headers: { token: token },
  });
  return response.data;
};
const useGitSupExamDetails = ({ id }) => {
  ...
  return [studentExamsLoading, studentExam, refetchStudenExam];
};

export default useGitSupExamDetails;
